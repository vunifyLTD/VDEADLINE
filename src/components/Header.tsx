  ...
  useEffect(() => {
    const fetchStarCount = async () => {
      try {
        setIsLoading(true);
        setError(false);
        const response = await fetch('https://api.github.com/repos/vunifyLTD/VDEADLINE');
        if (!response.ok) {
      ...
      <header className="w-full bg-transparent">
        <div className="container mx-auto px-4 py-3">
          <div className="flex justify-between items-center">
            ...
            {/* GitHub Link */}
            <motion.a
                href="https://github.com/vunifyLTD/VDEADLINE"
                target="_blank"
            ...
            >
              ...
              <div className="hidden sm:flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-white/10">
                ...
                <motion.span
                    ...
                    className={`text-xs ${error ? 'text-red-400' : ''}`}
                >
                  {isLoading ? "..." : error ? "Error" : starCount}
                </motion.span>
              </div>
            </motion.a>
          </div>
